...
public:
	...
	virtual void show() {
		cout << this->getModel() << ", " << this->getManufacturer() << ", 남은 종이" << this->avCount() << ", 남은 잉크" << this->availableInk() << endl;
	}
};
...
public:
	...
	virtual void show() {
		cout << this->getModel() << ", " << this->getManufacturer() << ", 남은 종이" << this->avCount() << ", 남은 토너" << this->availableToner() << endl;
	}
};
...
int main() {
	...
	do {
		...
		cin >> id >> pages;
		if (id == 1) { ink->print(pages); }
		else if (id == 2) { laser->print(pages); }
		else {
